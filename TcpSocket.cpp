...
TcpSocket::Create()
{
  /*Creates the socket for TCP connections..
    ...
    {
        Logger::Instance().WriteError(log<<"Error creading TCP socket"<<WSAGetLastError());
        return 0;
    }
	Logger::Instance().WriteLog(log<<"TCP Socket creaded");
	return 1;
}
...
TcpSocket::Bind(int Port)
{
    ...
    {
        Logger::Instance().WriteError(log<<"Error binding TCP socket"<<WSAGetLastError());
        return 0;
    }
	Logger::Instance().WriteLog(log<<"Socket Binded");
	return 1;
}
...
{	
    ...
    if (iResult == SOCKET_ERROR) {
        Logger::Instance().WriteError(log<<"listen faild"<<WSAGetLastError());
       
        return 0;
    }
	Logger::Instance().WriteLog(log<<"Listening to connection");
	return 1;
...
TcpSocket::Accept()
{
	...
	int iAddrSize = sizeof(m_server);
        ...
        {
		Logger::Instance().WriteError(log<<"Accept failed:"<<WSAGetLastError());
			return 0;
        }
		 Logger::Instance().WriteLog(log<<"New Connection Accepted:"<<m_Listen);
		 return 1;
}
...
{	
   ...
   m_server.sin_port=htons(Port);
    ...
    if (m_server.sin_addr.s_addr == INADDR_NONE)
    {
        ...
        if (host == NULL)
        {
            Logger::Instance().WriteError(log<<"unable to resolve server");
            return 0;
        }
    ...
    {
	Logger::Instance().WriteError(log<<"Error Connect failed:"<<WSAGetLastError());
        return 0;
    }
	Logger::Instance().WriteLog(log<<"Connected to server");
	return 1;
}
...
TcpSocket::Close()
{
	...
	Logger::Instance().WriteLog(log<<"Closing the Socket");
	return 1;
}
